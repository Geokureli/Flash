package baseball.scenes.editor {
	...
	public class EditorScene extends Scene{
		...
		override protected function addStaticChildren():void {
			...
			// --- TOOLBOX
			place( assets.add(new Btn(), "btn_bomb",	"tools"), { x: 10, y:10, upFrame:0, selectedFrame:5 } );
			place( assets.add(new Btn(), "btn_rock",	"tools"), { x: 80, y:10, upFrame:1, selectedFrame:6 } );
			place( assets.add(new Btn(), "btn_block",	"tools"), { x:150, y:10, upFrame:2, selectedFrame:7 } );
			place( assets.add(new Btn(), "btn_gap",	"tools"), { x:220, y:10, upFrame:3, selectedFrame:8 } );
			(place( assets.add(new Btn(), "btn_remove","tools"), { x:290, y:10, upFrame:4, selectedFrame:9 } ) as Button).selected = true;
			placeOnLayer("back", assets.add(new Staff(), "staff"));
			
			// --- NAVIGATION BUTTONS
			place( add(new Btn(Imports.navButtons), "btn_left2")	,{ x:360, y:58, upFrame:6, overFrame:7, downFrame:8 } );
			place( add(new Btn(Imports.navButtons), "btn_left")		,{ x:380, y:58, upFrame:3, overFrame:4, downFrame:5 } );
			place( add(new Btn(Imports.navButtons), "btn_right")	,{ x:400, y:58, upFrame:0, overFrame:1, downFrame:2 } );
			place( add(new Btn(Imports.navButtons), "btn_right2")	,{ x:420, y:58, upFrame:9, overFrame:10,downFrame:11} );
			
			
			// --- MISC BUTTONS
			place( add(new Btn(), "btn_music")	,{ x:510, y:10, upFrame:13, overFrame:18 } );
			place( add(new Btn(), "btn_save")	,{ x:580, y:10, upFrame:10, overFrame:15 } );
			place( add(new Btn(), "btn_load")	,{ x:650, y:10, upFrame:11, overFrame:16 } );
			place( add(new Btn(), "btn_upload")	,{ x:720, y:10, upFrame:12, overFrame:17 } );
			place( add(new Btn(), "btn_play")	,{ x:10, y:325, upFrame:14, selectedFrame:19 } );
			place( add(new Btn(Imports.TestBtn), "btn_test") ,{ x:650, y:325, upFrame:0, overFrame:1, downFrame:2 } );
			place( add(new Btn(Imports.smlButtons), "btn_metronome") ,{ x:360, y:10, upFrame:0, selectedFrame:1 } );
			//place("front", add(new Btn(Imports.smlButtons), "btn_playback") ,{ x:650, y:325, upFrame:0, downFrame:1 } );
			
			//place( "front", add(new Slider(5, 20, 1, 3), "sld_speed")) {x:100, y:375, label:"speed", width:50, value:-Obstacle.SCROLL }
			
			//place( "front", add(new Slider(2, 8, 1, 1), "sld_meter")) {x:175, y:375, label:"meter", width:50, value:staff.meter, valueSuffix:"/8" }
			
			//place( "front", add(new Slider(60, 160, 0, 10), "sld_bpm")){x:250, y:375, label:"beats/minute", width:150, value:BeatKeeper.beatsPerMinute, allowTextEdit:true, dragInterval:1 }
			
			//place( "front", add(new Slider(0, 1, 0, .25), "sld_offset")) {x:425, y:375, label:"offset(seconds)", width:150, value:songOffset/1000, precision:3, allowTextEdit:true, dragInterval:.01 }
		...
		private function loadLevel(level:XML):void {
			...
			for each(var child:XML in level.assets[0].children()) {
				...
				switch(child.name().toString()) {
					case "ball":	ob = place(add(new Bomb ()), PARAMS) as Obstacle; break;
					case "gap":		ob = place(add(new Gap  ()), PARAMS) as Obstacle; break;
					case "rock":	ob = place(add(new Rock ()), PARAMS) as Obstacle; break;
					case "block":	ob = place(add(new Block()), PARAMS) as Obstacle; break;
				}
				ob.beat = beat;
//				ob.update();
				ob.debugDraw();
