...
pathPropsText = "%%ifproplength%%%%proploop%%\n"..tab3
				.."%propname%=\"%propvalue%\""
				.."%%proploopend%%%%endifproplength%%"

pathText = tab3.."<nodes>\n%nodelist%"..tab3.."</nodes>\n"..tab2.."</path>\n"
linesText = pathIdText..tab2.."<path spline=\"false\" closed =\"%isclosed%\""
			..tab3..linkAssignText
			..pathPropsText.."\n"..tab2..">\n"..pathText
curvesText = pathIdText..tab2.."<path spline=\"true\" closed =\"%isclosed%\""
			..tab3..linkAssignText
			..pathPropsText.."\n"..tab2..">\n"..pathText
...
-------------------------------------
shapePropsText = "%%ifproplength%%%%proploop%%\n"..tab3.."%propname%=\"%propvalue%\"%%proploopend%%"
shapePropsText = shapePropsText.."\n"..tab2.."/>%%endifproplength%%%%ifnoproplength%%/>%%endifproplength%%\n"
...
spritePropsText = "%%ifproplength%%"
	spritePropsText = spritePropsText.."%%proploop%%\n"..tab3.."%propname%\=\"%propvalue%\"%%proploopend%%"..
					"%%endifproplength%%"
spriteText = tab2.."<sprite type=\"%class%\" "..
			...
			"yScale=\"%scaley%\" "..
			"%%if parent%%"..
				"\n"..tab3.."pathId=\"%getparent%\" ".."startNode=\"%attachedsegment%\" pathT=\"%attachedsegment_t%\""..
			"%%endifparent%%"..
...
for groupIndex = 0,groupCount do
	...
	for layerIndex = 0,layerCount do
		...
		if isMap == true then
			mapFileName = "maps".."/"..groupName.."/"..layerName..".csv"
			...
			if exportOnlyCSV == false then
				...
				layerText = layerText..tab2..
							...
							..tab3.."x=\""..string.format("%.3f",x).."\" "
								.."y=\""..string.format("%.3f",y).."\"\n"
							..tab3.."tileWidth=\""..as3.tolua(layer.map.tileWidth).."\" "
								.."tileHeight=\""..as3.tolua(layer.map.tileHeight).."\"\n"
							..tab3.."collide=\""..hasHitsString.."\"\n"
							..tab3.."collIdx=\""..as3.tolua(layer.map.collideIndex).."\" "
								.."drawIdx=\""..as3.tolua(layer.map.drawIndex).."\""
							..as3.tolua(DAME.GetTextForProperties(
								"%%ifproplength%%%%proploop%%\n"..tab3
								.."%propname%=\"%propvalue%\""
								.."%%proploopend%%\n".."%%endifproplength%%",
								layer.properties
							))
	...
	if exportOnlyCSV == false then
		...
		for i,v in ipairs(layers) do
			...
			fileText = fileText..tab1
					.."<layer name=\""..layerName.."\" xScroll=\""..string.format("%.3f",xscroll).."\" yScroll=\""..string.format("%.3f",yscroll).."\""
					..as3.tolua(DAME.GetTextForProperties( layerPropsString, layer.properties ))..">\n"
					..layers[i][4]..tab1.."</layer>\n"
		end
