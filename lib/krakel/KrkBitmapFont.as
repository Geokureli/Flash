package krakel {
	/**
 * KrkBitmapFont, stolen from FlxBitmapFont
...
*/
	...
	public class KrkBitmapFont extends FlxSprite
	{
		/**
		...
		protected var padding:FlxPoint = new FlxPoint();
		 ...
		 */
        public function KrkBitmapFont(font:Class, characterWidth:uint, characterHeight:uint, chars:String, charsPerRow:uint, xSpacing:uint = 0, ySpacing:uint = 0, xOffset:uint = 0, yOffset:uint = 0):void
        {
		 ...
		 * @return	void
		 */
		protected function buildBitmapFontText():void
		{
			var temp:BitmapData;
			var cx:int = 0;
			var cy:int = 0;
		 ...
		 */
		protected function pasteLine(output:BitmapData, line:String, x:uint = 0, y:uint = 0, customSpacingX:uint = 0):void
		{
			for (var c:uint = 0; c < line.length; c++)
			{
				...
				else if (grabData[line.charCodeAt(c)] is Rectangle)
				{
					output.copyPixels(fontSet, grabData[line.charCodeAt(c)], new Point(x, y));
					
					x += grabData[line.charCodeAt(c)].width + customSpacingX + padding.x;
