...
package mochi.as3 {
    ...
    public class MochiAd {
        ...
        public static function loadDock(options:Object):void {
            ...
            */
            ...
            var DEFAULTS:Object = {
                ...
                regpt: "o",
                method: "showDock",
                gameWidth:clip.stage.stageWidth,
                gameHeight:clip.stage.stageHeight,
                ad_count:5,
                ad_loaded: function (width:Number, height:Number):void { },
            ...
            options = MochiAd._parseOptions(options, DEFAULTS);
            trace('[MochiAd] showDock position :', options.position);
            if (POSITIONS[options.position] == null) {
               throw new Error('[MochiAd] Invalid showDock position property');
            }
            /* Load targeting under clip._mochiad */
            if (!MochiAd.load(options)) {
                options.ad_failed();
        ...
        public static function loadShowcase(options:Object):void {
            ...
            */
            ...
            var DEFAULTS:Object = {
                ...
                ad_count:5,
                gameWidth:clip.stage.stageWidth,
                gameWidth:clip.stage.stageHeight,
                ad_loaded: function (width:Number, height:Number):void { },
            ...
            var Pos:Array = options.position.split("x");

            var invalidPosition:Boolean = Pos.length != 2;
            for (var i:Number = 0; i< Pos.length; i++) {
               if (!invalidPosition) {
                   invalidPosition = String(parseInt(Pos[i])) != String(Pos[i]);
                }
            }
            if (invalidPosition) {
               throw new Error('[Mochiad] Invalid offset position for showcase. Please use XxY format where X and Y are positive or negative integer values.');
            }
        ...
        {
            if (clip && clip.stage){
                clip.stage.dispatchEvent(new Event("Mochiad:openAdGroup"));
            }
        }
        ...
        {
            if (clip && clip.stage){
                clip.stage.dispatchEvent(new Event("Mochiad:closeAdGroup"));
            }
        }
        ...
        {
            if (clip && clip.stage){
                clip.stage.dispatchEvent(new Event("Mochiad:openDockSlider"));
            }
        }
